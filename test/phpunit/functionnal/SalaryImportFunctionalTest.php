...
/**
 * \file    test/phpunit/SalaryImportFunctionalTest.php
...
/**
 * Class SalaryImportFunctionalTest
 *
 ...
 *
 * @package Testsalaryimport
 */
class SalaryImportFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	private static $module_id = 510000; // TODO: autodetect?
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modSalaryImport')->click();
		} else {
			// Disable the module
			$this->byHref('modSalaryImport')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/salaryimport/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('salaryimport/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/salaryimport/admin/about.php');
		$this->authenticate();
		return $this->assertContains('salaryimport/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('salaryimportwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modSalaryImport_SalaryImportTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modSalaryImport_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
